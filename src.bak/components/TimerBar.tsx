...
/**
 * A deterministic progress bar driven by requestAnimationFrame.
 * Uses server timestamps so all clients stay in perfect sync.
 * Updates the DOM directly (no React state) for maximum smoothness.
 */
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (barRef.current) {
        ...
        // Colour shift: violet → amber → red as time runs out
        if (remaining < 0.15) {
          ...
          barRef.current.style.backgroundColor = '#7c3aed'; // violet-700
        }
      }
      ...
      <div className="h-1.5 w-full bg-white/10">
        ...
        <div
          ...
          style={{
            ...
            willChange: 'width',
            // No transition on width — RAF handles smoothness
          }}
        />
      </div>
    </div>
